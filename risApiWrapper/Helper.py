...
def _request(url, parameters) -> list:
    '''
    Requests a response from the provided api with the provided parameters. If the response has 100 entries, it continues to request additional "pages" as long as a response with less than 100 entries is returned. A list of all entries is returned.
    '''
...
def _sort_results(results: list, sort_key: str, sort_keys: list, ascending: bool) -> list:
    '''
...
def _input_validation(key: str, value: str, values: list) -> None:
    '''
    Validates the provided inputs against a list of possible inputs and raises an exception if not matching.
    '''
